...
- [x] WaitlistSlotDetectionJob runs as Hangfire recurring job every 2 minutes
- [x] WaitlistTimeoutJob runs as Hangfire recurring job every 1 minute
- [x] POST /api/waitlist/{token}/confirm endpoint books appointment when slot available (AC-2)
- [x] POST /api/waitlist/{token}/confirm returns 409 when slot re-booked (EC-2)
- [x] POST /api/waitlist/{token}/decline resets entry to Active and cascades (AC-3)
- [x] Both endpoints are [AllowAnonymous] for SMS/Email link access
- [x] AppointmentService.CancelAsync triggers immediate waitlist detection on slot release
