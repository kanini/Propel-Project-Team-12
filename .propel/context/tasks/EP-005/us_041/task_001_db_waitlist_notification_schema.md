...
- [X] Add `NotifiedAt`, `ResponseToken`, `ResponseDeadline`, `NotifiedSlotId` nullable fields to `WaitlistEntry` model
- [X] Add `NotifiedSlot` navigation property with FK to TimeSlot
- [X] Configure `ResponseToken` as varchar(64) with unique filtered index in WaitlistEntryConfiguration
- [X] Add partial index on `(Status, ResponseDeadline)` filtered to Notified status for timeout job
- [X] Generate and verify EF Core migration `AddWaitlistNotificationFields`
