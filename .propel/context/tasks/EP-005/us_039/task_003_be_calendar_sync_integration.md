...
- [x] CalendarSyncJob compiles with all three async methods
- [x] `[AutomaticRetry]` configured with 3 attempts and 1min/4min/16min delays
- [x] CreateCalendarEventAsync persists GoogleCalendarEventId on Appointment after creation
- [x] CreateCalendarEventAsync silently skips when user has no calendar connection
- [x] UpdateCalendarEventAsync skips when no GoogleCalendarEventId present
- [x] AppointmentService.CreateAppointmentAsync enqueues calendar sync job after commit
- [x] AppointmentService.RescheduleAsync enqueues calendar update job after commit
- [x] AppointmentService.CancelAsync enqueues calendar delete job with captured event ID

## Implementation Checklist

- [x] Create `CalendarSyncJob` with `CreateCalendarEventAsync`, `UpdateCalendarEventAsync`, `DeleteCalendarEventAsync` methods
- [x] Configure `[AutomaticRetry]` with exponential backoff (1min, 4min, 16min) on all job methods
- [x] Implement event creation logic with GoogleCalendarEventId persistence and connection check
- [x] Integrate `BackgroundJob.Enqueue<CalendarSyncJob>` into `CreateAppointmentAsync` after transaction commit
- [x] Integrate `BackgroundJob.Enqueue<CalendarSyncJob>` into `RescheduleAsync` after transaction commit
- [x] Integrate `BackgroundJob.Enqueue<CalendarSyncJob>` into `CancelAsync` after cancellation with captured event ID
- [x] Register `CalendarSyncJob` as Scoped in `Program.cs` for Hangfire DI resolution
