...
- [ ] calendarApi.ts exports getStatus, getConnectUrl, disconnect functions
- [ ] ConfirmationDialog checks calendar connection status on mount
- [ ] Connected users see "Synced to Google Calendar" with checkmark
- [ ] Disconnected users see "Connect Google Calendar" button that triggers OAuth redirect
- [ ] OAuth return handled via URL parameter detection
- [ ] Outlook button remains unchanged (static URL)
- [ ] Loading state shown during calendar status check (UXR-501)

## Implementation Checklist

- [ ] Create `calendarApi.ts` with getStatus, getConnectUrl, disconnect API functions
- [ ] Add `googleCalendarSynced` optional field to `Appointment` TypeScript interface
- [ ] Add calendar connection status check on `ConfirmationDialog` mount with loading state
- [ ] Implement conditional rendering: connected (synced badge), disconnected (connect button), loading (skeleton)
- [ ] Implement OAuth flow trigger via `handleConnectGoogleCalendar` using window redirect
- [ ] Handle OAuth return by detecting `calendar_connected` URL parameter
- [ ] Preserve existing Outlook Calendar static URL button unchanged
