...
- [x] CalendarIntegration entity compiles with all required properties
- [x] Unique composite index on (UserId, Provider) prevents duplicate connections
- [x] AccessToken and RefreshToken columns sized for encrypted tokens (2048)
- [x] GoogleCalendarEventId added to Appointment as nullable string (varchar 256)
- [x] DbSet<CalendarIntegration> registered in PatientAccessDbContext
- [x] EF Core migration generates without errors

## Implementation Checklist

- [x] Create `CalendarIntegration` entity with UserId, Provider, encrypted token fields, IsConnected flag
- [x] Create `CalendarIntegrationConfiguration` with unique composite index and column constraints
- [x] Add `GoogleCalendarEventId` nullable string property to `Appointment` model
- [x] Update `AppointmentConfiguration` with column config for GoogleCalendarEventId
- [x] Add `DbSet<CalendarIntegration>` to `PatientAccessDbContext` and apply configuration in `OnModelCreating`
