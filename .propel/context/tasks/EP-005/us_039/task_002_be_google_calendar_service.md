...
- [x] ICalendarService interface compiles with all 7 methods
- [x] GoogleCalendarService builds with Google.Apis.Calendar.v3 dependency resolved
- [x] OAuth2 authorization URL includes correct scopes, access_type=offline, prompt=consent
- [x] Callback handler exchanges code for tokens and stores encrypted in CalendarIntegration
- [x] Token refresh logic updates AccessToken on expiry; marks IsConnected=false on refresh failure (EC-2)
- [x] CreateEventAsync returns Google event ID for storage
- [x] UpdateEventAsync and DeleteEventAsync operate on correct event via eventId
- [x] CalendarController endpoints are protected by [Authorize] except callback

## Implementation Checklist

- [x] Add `Google.Apis.Calendar.v3` NuGet package to PatientAccess.Business.csproj
- [x] Create `CalendarEventDto` with title, start/end time, description, location fields
- [x] Create `ICalendarService` interface with CRUD and OAuth methods
- [x] Implement `GoogleCalendarService` OAuth2 flow (authorization URL generation, callback token exchange)
- [x] Implement token refresh with automatic retry and IsConnected=false on failure
- [x] Implement calendar event CRUD (CreateEventAsync, UpdateEventAsync, DeleteEventAsync) using Google API
- [x] Create `CalendarController` with connect, callback, status, disconnect endpoints
- [x] Register ICalendarService in DI, add GoogleCalendarSettings to appsettings.json, add named HttpClient
