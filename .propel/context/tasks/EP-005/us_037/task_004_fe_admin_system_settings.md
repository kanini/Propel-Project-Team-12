...
- [ ] SystemSettingsPage renders Default state with form fields
- [ ] Loading state shows spinner/skeleton during API fetch
- [ ] Error state shows error banner with retry on API failure
- [ ] Validation state shows inline errors for invalid input
- [ ] Settings save successfully and show confirmation feedback
- [ ] Admin-only route protection verified (non-admin redirected)
