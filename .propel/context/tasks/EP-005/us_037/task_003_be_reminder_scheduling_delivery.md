...
- [x] `IReminderService` interface compiles with all three methods
- [x] `ReminderService.ScheduleRemindersAsync` creates correct number of Notification records (intervals × channels)
- [x] `ReminderService.CancelRemindersAsync` sets Pending notifications to Cancelled status
- [x] `ReminderSchedulerJob` runs without errors and enqueues due notifications
- [x] `ReminderDeliveryJob` retries with correct delays (60s, 240s, 960s) on failure
- [x] `GET /api/admin/settings` returns current system settings (Admin role required)
- [x] `PUT /api/admin/settings` updates settings values
- [x] Appointment booking triggers reminder scheduling
- [x] Appointment cancellation cancels pending reminders
