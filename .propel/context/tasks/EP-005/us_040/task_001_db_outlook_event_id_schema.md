...
- [ ] Add `OutlookCalendarEventId` nullable string property to `Appointment` model after `GoogleCalendarEventId`
- [ ] Update `AppointmentConfiguration` with `HasMaxLength(256)` for OutlookCalendarEventId
- [ ] Generate and verify EF Core migration `AddOutlookCalendarEventId`
- [ ] Confirm CalendarIntegration entity supports "Outlook" provider value (no changes needed)
