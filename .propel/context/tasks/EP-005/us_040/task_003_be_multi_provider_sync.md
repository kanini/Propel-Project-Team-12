...
- [ ] Replace single `ICalendarService` DI registration with `AddKeyedScoped` for "Google" and "Outlook"
- [ ] Refactor `CalendarSyncJob` constructor to inject `IServiceProvider` instead of single `ICalendarService`
- [ ] Refactor `CreateCalendarEventAsync` to iterate connected providers and persist per-provider event IDs
- [ ] Refactor `UpdateCalendarEventAsync` to update events in both Google and Outlook when IDs exist
- [ ] Refactor `DeleteCalendarEventAsync` to accept both Google and Outlook event IDs and delete independently
- [ ] Update `AppointmentService.CancelAsync` to pass both event IDs to the delete job
- [ ] Refactor `CalendarController` with `{provider}` route parameter and multi-provider status endpoint
- [ ] Extract `BuildCalendarEventDto` helper for DRY event data construction
