...
- [ ] Update `calendarApi.ts` to accept provider parameter on `getConnectUrl` and `disconnect`
- [ ] Update `CalendarStatusResponse` to include both `google` and `outlook` status
- [ ] Add `outlookCalendarSynced` optional field to the Appointment TypeScript interface
- [ ] Replace `getOutlookCalendarUrl()` static function with OAuth-aware `handleConnectOutlook`
- [ ] Add `outlookConnected` and `outlookConnecting` state to ConfirmationDialog
- [ ] Render Outlook button with three states: not-connected, connecting (loading), synced
- [ ] Handle Microsoft OAuth return via URL parameter detection on component mount
