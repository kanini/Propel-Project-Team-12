...
<body>
        ...
        string location)
    {
        try
        {
            ...
            var sendGridEnabled = bool.TryParse(_configuration["SendGridSettings:Enabled"], out var enabled) && enabled;
            var sendGridApiKey = _configuration["SendGridSettings:ApiKey"];

            // Development mode: log instead of sending
            if (!sendGridEnabled || string.IsNullOrWhiteSpace(sendGridApiKey))
            {
                _logger.LogInformation(
                    "APPOINTMENT REMINDER EMAIL (Development Mode)\n" +
                    "To: {Email}\n" +
                    "Name: {Name}\n" +
                    "Provider: {Provider}\n" +
                    "Scheduled: {DateTime}\n" +
                    ...
                    "\n" +
                    "Manage your appointments: {ManageLink}\n" +
                    "\n" +
                    "Contact: (555) 123-4567 or support@patientaccess.com",
                    toEmail, toName, providerName,
                    scheduledDateTime.ToString("MMMM dd, yyyy 'at' h:mm tt"),
                    location, manageAppointmentLink);

                await Task.Delay(100); // Simulate async sending
                return true;
            }

            // Production mode: Send via SendGrid
            // TODO: Implement SendGrid SDK integration when SendGrid package is verified
            // var client = new SendGridClient(sendGridApiKey);
            // var from = new EmailAddress(_configuration["SendGridSettings:FromEmail"], _configuration["SendGridSettings:FromName"]);
            // var to = new EmailAddress(toEmail, toName);
            // var subject = $"Reminder: Appointment with {providerName}";
            // var plainTextContent = [formatted reminder text];
            // var htmlContent = [formatted HTML reminder];
            // var msg = MailHelper.CreateSingleEmail(from, to, subject, plainTextContent, htmlContent);
            // var response = await client.SendEmailAsync(msg);

            _logger.LogInformation("Appointment reminder email sent to {Email} via SendGrid", toEmail);
            await Task.Delay(100); // Placeholder for actual API call
            return true;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to send appointment reminder email to {Email}", toEmail);
            return false;
        }
    }
        ...
        int timeoutMinutes)
    {
        try
        {
            ...
            {
                _logger.LogInformation(
                    "WAITLIST SLOT NOTIFICATION EMAIL (Development Mode)\n" +
                    "To: {Email}\n" +
                    "Name: {Name}\n" +
                    "Provider: {Provider}\n" +
                    "Slot Available: {DateTime}\n" +
                    "Timeout: {TimeoutMinutes} minutes (expires at {ExpiryTime})\n" +
                    "Email Body:\n" +
                    "Dear {Name}, great news! A time slot with {Provider} has become available.\n" +
                    "\n" +
                    "Appointment Details:\n" +
                    "Provider: {Provider}\n" +
                    "Date & Time: {DateTime}\n" +
                    "\n" +
                    "ACTION REQUIRED:\n" +
                    "Please respond within {TimeoutMinutes} minutes (by {ExpiryTime}).\n" +
                    "\n" +
                    "✅ CONFIRM this appointment: {ConfirmUrl}\n" +
                    "❌ DECLINE this offer: {DeclineUrl}\n" +
                    "\n" +
                    "If you don't respond within {TimeoutMinutes} minutes, this offer will expire and the slot will be offered to the next patient on the waitlist.\n" +
                    "\n" +
                    "Contact: (555) 123-4567 or support@patientaccess.com",
                    toEmail, toName, providerName,
                    slotDateTime.ToString("MMMM dd, yyyy 'at' h:mm tt"),
                    timeoutMinutes, expiryTime.ToString("h:mm tt"),
            ...
            // var from = new EmailAddress(_configuration["SendGridSettings:FromEmail"], _configuration["SendGridSettings:FromName"]);
            // var to = new EmailAddress(toEmail, toName);
            // var subject = $"Time Slot Available with {providerName}";
            // var plainTextContent = [formatted notification text];
            // var htmlContent = [formatted HTML notification with prominent action buttons];
            // var msg = MailHelper.CreateSingleEmail(from, to, subject, plainTextContent, htmlContent);
            // var response = await client.SendEmailAsync(msg);

            _logger.LogInformation("Waitlist slot notification email sent to {Email} via SendGrid", toEmail);
            await Task.Delay(100); // Placeholder for actual API call
            return true;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to send waitlist slot notification email to {Email}", toEmail);
            return false;
        }
    }
}
